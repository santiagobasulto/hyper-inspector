...
class FileLogger(BaseLogger):
    def log(self, request):
        ...
        if not self.args.logging_directory.exists():
            rich.print(f"[red bold]Error: Can't write file logs because directory [italic]{str(self.args.logging_directory)}[/italic] doesn't exist")
            return
        path_name = ".".join(request.clean_path.split("/")[1:]) or "(root)"
        base_file_name = f"{request.method.upper()}.{path_name}.{int(datetime.utcnow().timestamp())}"
